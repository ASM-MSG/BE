...
- **커밋 포인트 제안 (중간중간, 필수)**: 모듈이 green + 리뷰를 통과할 때마다 리더는 그 시점의
  ...
  커밋 실행은 항상 사용자 몫이다.
  **구현이 한 에이전트 런에 몰린 경우에도 몰아 커밋 금지** — 개발 에이전트가 여러 모듈을 한 번에
  완성해 중간 포인트가 없었으면, 리더가 사후에라도 레이어/모듈 단위(엔티티·리포지토리 → 서비스·DTO
  → 컨트롤러 → 문서, 각 단계 독립 컴파일 순서)로 **경로 스테이징 커밋 계획을 쪼개 제안**한다.
  파일 수가 3~4개를 넘는 단일 커밋 제안은 그 자체가 신호다 (MSG-222에서 17파일 1커밋 제안 → 지적받음,
  2026-07-24).
