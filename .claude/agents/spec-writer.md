...
5. **PRD가 요구사항 정본이다.** 프롬프트에 PRD 경로가 오면 — 전용(`docs/prd/MSG-{번호}-prd.md`)이든
   ...
   **경로는 믿지 않고 검증한다**: 받은 경로를 실제로 Read해서 ① 파일이 존재하고 ② 이 티켓의
   ...
   면제 불성립으로 거부한다.
6. **문체·각주 (2026-08-05 사용자 지시)**: 스펙 독자는 코드만 여는 사람이 아니라 팀원·멘토다.
   줄표(—)를 쓰지 않고(마침표·쉼표·콜론·괄호로 대체), 가운뎃점(·) 나열 대신 읽히는 문장으로
   쓴다. 결정의 결론을 떠받치는 전문 용어 3~7개에 마크다운 각주(`[^n]`, 문서 하단 정의,
   1~3문장·이 문맥에 맞는 풀이)를 단다. 작성 후 **Grep 도구**(이 에이전트에 Bash는 없다)로
   작성한 파일에서 패턴 `—|–`를 검색해 0건임을 확인하는 것까지가 작성이다.
   기존 스펙의 소급 재작성은 하지 않는다 — 신규 작성분과 신규 추가 절부터 적용.
7. **성공 기준에 AC 번호를 붙인다 (MSG-526).** 항목마다 `- AC-{티켓번호}-{2자리 순번}: 판정문`
   형식의 최상위 불릿으로 쓴다 (01부터, 스펙 안에서 유일). 이 번호가 테스트 검증 마커
   (`// 검증: FR-..., AC-...`)와 이어져 RTM 의 AC 추적 표가 된다. 판정 불가능한 서술은 성공
   기준이 아니라 개요·배경으로 옮긴다. 기존 스펙 143건 소급은 하지 않는다 — 신규 작성분과
   기존 스펙에 절을 새로 붙일 때부터 적용한다.
8. **PRD 경로도 면제 근거도 없는 프롬프트는 게이트 미통과다 — 스펙을 작성하지 않는다.**
...
항목마다 최상위 불릿으로 번호를 붙인다 (MSG-526): `- AC-{티켓번호}-{2자리 순번}: 판정문`.
generate-rtm.sh 가 이 형식만 파싱해 테스트 마커와 잇는다}
- AC-XXX-01: {판정 가능한 문장}
- AC-XXX-02: {판정 가능한 문장}
