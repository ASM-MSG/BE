...
| 2026-09-08 | backend-audit 스킬 신설(MSG-583) — 렌즈 8개(native·plan·tx·osiv·tz·calls·naming·iface)를 병렬 서브에이전트로 전수 점검하고 `docs/audit/{날짜}-backend-audit.md` 검토 보고서를 만든다. 실측 스크립트 3개(inventory·capture-sql·explain) 동봉, 코드 수정·티켓 발행 없음 | skills/backend-audit(신설), CLAUDE.md | 2026-09-07 박원형 멘토링 — 멘토 지적(트랜잭션 안 S3·OSIV·native 108건·호출 관계·명명)이 일부 코드 기준이라 레포 전체에서 같은 패턴을 찾고 사실/추측을 가려야 했다. 기존 convention-reviewer는 스타일·경계 계약 체크리스트라 "실제 실행 SQL·커넥션 점유" 같은 동작 질문을 못 본다. 반복 실행(10-08 다음 멘토링 전 전후 비교)이 목적이라 일회성 감사 대신 스킬로. 실측은 Hibernate 로그가 아니라 DB `log_min_duration_statement`로 — 소스와 실행문이 다를 수 있고 N+1은 실행 횟수에서만 보인다 |
