...
public class EventLocation {
	 ...
	 * 중지가 격자 클레임을 반납하므로 대표 격자가 가리킬 행이 남지 않는다 (V55, MSG-598). 등식
	 * {@code (hidden_at IS NULL) = (representative_grid_id IS NOT NULL)} 을
	 * {@code chk_event_loc_rep_grid_visible} 가 DB 에서 지킨다 — 숨기면서 대표 격자를 남기는 쓰기는
	 * 거부된다. 조회 경로는 모두 숨긴 위치를 걸러 내므로 응답 계약은 종전대로 non-null 이다.
	 */
