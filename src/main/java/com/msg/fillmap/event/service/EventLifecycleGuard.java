...
public class EventLifecycleGuard {
	...
	/**
	 * <b>지금 규칙 (FR-13)</b> — 댓글·도움돼요 변경은 <b>아카이브 전환(종료 + 30일)부터</b> 잠긴다.
	 * <ul>
	 * <li>예정 · 진행 중 · <b>유예 기간(종료 ~ 종료 + 30일)</b> → 전부 열려 있다</li>
	 * <li>아카이브(종료 + 30일 이후) → {@code EVENT_INTERACTION_LOCKED}</li>
	 * </ul>
	 * 업로드 창(시작 ~ 종료 + 30일)과 구간이 정렬돼, 두 규칙이 같은 창을 보므로 "올릴 수는 있는데 아무도
	 * 반응 못 하는 영상"이 생기지 않는다.
	 * <p>
	 * <b>[폐기된 옛 규칙] "종료 정각부터 잠금"</b> — 아래 문단은 <b>이력이지 현재 동작이 아니다.</b>
	 * MSG-442 가 "종료 = 기록 확정"을 근거로 그렇게 만들었으나 <b>2026-08-21 번복됐다</b>(정민 확정,
	 * MSG-441 구현 중). 유예 기간을 둔 목적이 "행사 다녀와서 나중에 올리기"인데 그 영상이 처음부터 반응을
	 * 못 받으면 유예의 목적과 결과가 서로 깎인다는 지적으로 뒤집혔고, 기록 확정 시점은 아카이브 전환으로
	 * 옮겨갔다. 옛 규칙을 정본으로 적은 MSG-442 스펙 서술은 그 문서의 작업 로그가 무효를 표시한다.
	 */
