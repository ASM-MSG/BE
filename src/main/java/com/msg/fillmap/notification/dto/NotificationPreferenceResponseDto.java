 ...
 * 카테고리 8종을 enum 선언 순으로 반환한다 — 저장 행 없는 카테고리는 true 합성이라 FE 는 부재
 * 처리가 필요 없다. MODERATION 은 수신 거부 불가(MSG-417 FR-7)라 목록에 없다. 응답의 category 는
 * enum 직렬화(출력엔 파싱 실패가 없다).
 */
@Schema(description = "알림 설정 — 전 카테고리(8종)의 수신 상태 (저장 행 없는 카테고리는 true)",
	requiredProperties = {"preferences"})
public record NotificationPreferenceResponseDto(
	@Schema(description = "카테고리별 수신 상태 (BADGE·HOTZONE·REMIND·VIDEO·WEEKLY·FRIEND·MISSION_NEARBY·EVENT "
		+ "고정 8종 — MODERATION 은 설정 대상이 아니라 없다)")
