...
/**
 * 역지오코딩 응답 (MSG-93). 포함 행정동이 없으면 data 자체가 null 이다(§D3). boundary_geom(경계 폴리곤)은
 * 무겁고 검색 UX 에 불필요해 넣지 않는다(YAGNI) — 경계가 필요한 화면이 생기면 GeoJSON opt-in 을 추가한다.
 */
@Schema(description = "좌표를 포함하는 행정동. 포함 행정동이 없으면(바다·국외) data 가 null 이다.",
	requiredProperties = {"regionCode", "regionName", "parentCode"})
