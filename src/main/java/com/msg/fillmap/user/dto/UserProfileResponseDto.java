...
/**
 * 내 프로필 응답 (MSG-203). 소셜 로그인이 자동 저장한 값을 가공 없이 노출한다 — nickname 은
 * users NOT NULL 컬럼이라 항상 존재하지만, email 은 카카오 가입이 이메일을 수집하지 않아
 * null 일 수 있다 (V16 · MSG-310). 조회(GET /me)와 닉네임 수정(PUT /me/nickname)이 공유해
 * 응답 형태가 어긋날 수 없다 (§D2).
 */
