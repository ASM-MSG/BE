...
/**
 * 내 프로필 응답 (MSG-203). 소셜 로그인이 자동 저장한 값을 가공 없이 노출한다 — email·nickname 은
 * users NOT NULL 컬럼이라 항상 존재. 조회(GET /me)와 닉네임 수정(PUT /me/nickname)이 공유해
 * 응답 형태가 어긋날 수 없다 (§D2).
 */
@Schema(description = "내 프로필 응답. 조회·닉네임 수정이 같은 형태를 반환한다.",
	requiredProperties = {"email", "nickname"})
public record UserProfileResponseDto(
	@Schema(description = "가입 이메일 — 소셜 로그인(id_token 클레임)에서 자동 저장된 값", example = "user@fillmap.dev")
	String email,
