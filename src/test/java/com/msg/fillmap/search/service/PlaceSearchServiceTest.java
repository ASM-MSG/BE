 ...
 * 고정해 인코딩 규칙 회귀를 즉시 적발한다(§D7).
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("PlaceSearchService — trim 가드·gridId 합성·주소 규칙")
class PlaceSearchServiceTest {
	...
	@Test
	void q가_공백이면_카카오를_호출하지_않고_빈_리스트를_반환한다() {
	...
	void 좌표를_격자ID로_즉석_계산해_결과에_싣는다() {
		...
		List<PlaceSearchResponseDto> results = placeSearchService.searchPlaces("  강남역  ");
	...
	void 부산대_서면역_홍대입구역_광안리_좌표는_스모크_실측_격자ID와_일치한다() {
		...
		List<PlaceSearchResponseDto> results = placeSearchService.searchPlaces("스모크");
	...
	void 도로명주소가_있으면_도로명을_없으면_지번을_address로_쓴다() {
		...
		List<PlaceSearchResponseDto> results = placeSearchService.searchPlaces("주소");
