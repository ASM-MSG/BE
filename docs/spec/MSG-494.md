...
2. 각 회차에서 `claimed`의 `node` 분포에 `be`와 `ai`가 모두 나타나고, 작업 유실과 영상의 중복 종결은
   각각 0건이다.
3. 첫 번째와 두 번째 시도에서 워커 프로세스를 중단하면 임대 만료 또는 명시적 실패 반환 뒤 다른 노드가
   작업을 가져간다. 세 번째 처리 시도까지 실패한 영상은 FAILED가 되고 UPLOADED나 ENCODING에 남지 않는다.
...
영상 상태를 쓰는 모든 인코딩 경로는 작업 행을 먼저 잠그고 다음 조건을 모두 확인한다.
...
조건이 맞지 않으면 만료된 작업의 결과이므로 영상과 알림을 건드리지 않는다. 같은 토큰이 맞는 경우에는
ENCODING 시작은 작업 행과 영상 행 순서로 잠근다. 알림을 함께 쓰는 종결은 작업 행, 업로더 행, 영상 행
순서로 잠근 뒤 영상 전이와 작업 종결을 한 트랜잭션으로 커밋한다. 기존
`original_s3_key`와 처리 상태 가드도 그대로 적용한다. 토큰 가드는 실행 노드의 권한을 확인하고, 원본 키
가드는 영상 교체나 삭제로 현재 시도가 바뀌었는지 확인한다.
...
| JVM 또는 EC2 중단 | 임대가 끝난 PROCESSING을 다른 노드가 다시 선점 |
...
`original_s3_key`로 PENDING 작업을 한 번 보강한다. BLURRING, READY, FAILED 영상은 대상이 아니다.
...
모든 Micrometer 지표에 `node=${FILLMAP_NODE_ID:be}` 공통 태그를 추가한다. 새 지표는 고정된 태그 값만
사용한다.
...
| 기존 `video.processing.duration` Timer | `outcome`, `path`, 공통 `node` | DB 시작 시각 기준 전체 처리 시간 |
| 기존 `video.processing.outcome` Counter | `outcome`, `path`, 공통 `node` | 실제 영상 종결 수 |
...
WITH candidate AS (
    ...
    WHERE (status = 'PENDING' AND available_at <= CURRENT_TIMESTAMP)
       OR (status = 'PROCESSING' AND lease_until <= CURRENT_TIMESTAMP AND attempt_count < 3)
...
실제 구현은 PostgreSQL interval 문자열 조립을 피하고 Java가 계산한 `leaseUntil`을 바인딩한다. 같은 이유로
재시도 `availableAt`도 Java에서 UTC 시각을 계산해 넘긴다.
...
그 뒤 `completed_at IS NULL`인 DEAD 한 건을 `FOR UPDATE SKIP LOCKED`로 잡아 새 finalizer token과 임대를
기록한다. `VideoStatusWriter`가 그 token을 확인하고 영상 FAILED, 알림, `completed_at`을 한 트랜잭션으로
기록한다. finalizer가 중단돼도 임대가 끝나면 같은 절차를 반복한다. DEAD는 FFmpeg를 다시 실행하지 않는다.
...
4. 영상 전이, 처리 알림 outbox INSERT, 작업 COMPLETED, 종결 지표 기록을 한 트랜잭션으로 처리한다.
5. 하이라이트 후행 계산은 READY 이후의 베스트 에포트 동작을 유지한다. 실패해도 완료 작업을 되돌리지 않는다.

### 7.4 잠금 순서

인코딩 시작은 `video_encoding_jobs -> videos`, 종결은
`video_encoding_jobs -> users -> videos` 순서로 잠근다. 기존 탈퇴와 알림 경로의
`users -> videos` 순서를 뒤집지 않으면서 claim을 먼저 고정하는 순서다. 작업 선점은 작업 행만 잠그고
...
`TIMESTAMP`는 레포의 V33 이후 규약대로 UTC 벽시계로 저장한다. 작업 행은 영상 삭제 시 함께 삭제한다.
완료 행은 처리 시간과 유실 검증 근거라 이번 티켓에서 정리 배치를 만들지 않는다. 보관량이 실제로 문제가
될 때 별도 보존 정책을 정한다.
...
2. 같은 JAR를 AI EC2의 별도 경로에 올리고 `fillmap-encoding-worker`를 재시작한다.
...
| `src/main/java/com/msg/fillmap/video/service/EncodingJobPoller.java` | 노드별 한 건 선점, 실행, 재시도와 DEAD 종결 |
...
| `src/main/java/com/msg/fillmap/video/service/VideoStatusWriter.java` | claim token 확인과 영상, 알림, 작업 종결의 원자적 커밋 |
| `src/main/java/com/msg/fillmap/video/service/VideoProcessingMetrics.java` | 인메모리 시작점 제거, DB 등록 시각과 node 태그 사용 |
...
| `src/main/resources/application.yml` | 작업 기본값, 기본 활성 gate, node 공통 태그 |
| `src/main/resources/application-encoding-worker.yml` | 워커 포트, 풀 크기, 시더와 health 차이 |
...
- `worker가_종료돼도_선점_작업은_임대_만료후_다른_노드가_회수한다`
...
- `잠금_순서는_작업_업로더_영상_순서다`

### 12.5 설정과 배포 테스트

- `EncodingWorkerContextTest`: `dev,encoding-worker`에서 job poller는 뜨고 `AiBlurPoller`와
  `EventNotificationScheduler`는 뜨지 않는다.
- `VideoProcessingMetricsTest`: DB 등록 시각 기준 duration과 `node` 태그, 고정 job event를 검증한다.
...
6. 블러를 켜고 기존 영상 한 건을 BLURRING으로 만든 상태에서 새 영상 두 건을 동시에 확정한다.
7. 종료 뒤 같은 시스템 지표를 저장하고 전후 차이를 이 문서 작업 로그에 붙인다.
...
구현 완료 뒤 아래 네 항목을 실제 값으로 갱신한다.

- **데이터 저장 위치**: `video_encoding_jobs` 행 수와 상태별 수, S3 원본과 산출물 키 예시
- **실행 쿼리**: claim, retry, terminal SQL의 실제 바인딩과 `EXPLAIN (ANALYZE, BUFFERS)`
- **예외 흐름**: 영구 실패, 재시도 실패, 임대 만료가 최종 영상 상태로 바뀌는 호출 경로
- **빈 동작**: 두 프로필에서 뜨는 poller와 scheduler 목록, executor와 트랜잭션 경계
- **실측**: 3개 동시 업로드 3회 결과, 작업 유실과 중복 종결 수, 두 노드의 CPU, 메모리, swap 전후
