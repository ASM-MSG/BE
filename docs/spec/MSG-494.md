...
`EncodingJobPoller`는 실행 중인 claim을 `AtomicReference`로 보관한다. `@PreDestroy`에서는 새 선점을
막고 현재 token이 여전히 PROCESSING일 때만 즉시 PENDING으로 돌린다. 통제된 종료는 처리 실패가 아니므로
`attempt_count`를 1 줄여 다음 노드가 같은 시도 번호로 다시 시작하게 한다. 이미 COMPLETED이거나 token이
바뀌었으면 UPDATE 0행으로 끝난다. SIGKILL과 EC2 장애처럼 종료 훅을 실행하지 못한 경우에만 35분 임대가
최후 회수선으로 동작한다.
...
- `SIGTERM이면_현재_claim을_즉시_반납하고_시도횟수를_소비하지_않는다`
...
7. SIGTERM 시험은 즉시 반납을 확인한다. SIGKILL 시험은 임대를 PT10S로 임시 오버라이드해 회수한 뒤
   기본 PT35M로 복구한다.
