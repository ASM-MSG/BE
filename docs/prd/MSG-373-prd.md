...
| FR-5 | 허용 형식은 jpg, png, webp 세 가지, 크기는 최대 5MB다. 형식이나 한도를 벗어난 업로드는 명확한 실패 응답을 받는다. 아이폰 기본 사진 형식(heic)은 서버가 받지 않고 클라이언트 변환으로 지원한다 (2026-08-11 재확정) | Must |
| FR-6 | 프로필 이미지를 제거해 기본 상태로 되돌릴 수 있다 | Must |

heic 파일은 브라우저가 직접 표시하지 못한다. 아이폰 사진 지원은 두 겹의 클라이언트 처리로
성립한다(2026-08-11 정민 재확정, 저장된 heic가 표시되지 않는 모순을 짚은 교차 리뷰가 계기):
FE의 파일 선택 accept 목록에서 heic를 빼 iOS가 알아서 JPEG로 변환하게 만들고, 그래도 새어
들어오는 원본 heic는 서버 검증이 거부한다. 저장되는 이미지는 항상 모든 브라우저에서 보인다.
...
- [x] 허용 형식과 크기: jpg, png, webp 최대 5MB. 아이폰 형식(heic)은 처음엔 허용 목록에
      넣기로 했으나, 저장해도 브라우저에 표시되지 않는 모순이 확인돼 서버 거부 + 클라이언트
      변환으로 재확정 (2026-08-11, FR-5에 반영)
