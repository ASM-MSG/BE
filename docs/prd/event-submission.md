...
> 개정: 2026-08-29 v2.2. 미해결 질문 1(참여형 하위 실체의 명칭, 지역축제·팝업 승인분의 노출
> 형태)을 사용자 확정으로 해소해 결정 기록으로 옮겼고, FR-8에 이벤트 참여형의 대표 위치 1곳
> 예외를 더했다. 개정 범위는 그 해소 반영에 한정한다
> 개정: 2026-09-05 v2.3. FR-6의 반려 통보를 수기에서 자동 메일로 바꿨다(MSG-575, 사용자 지시).
> 반려 즉시 요청자의 공식 이메일로 반려 사유를 담은 안내 메일이 나가고, 본문은 필맵 전용 서식이다.
> 개정 범위는 FR-6 한 행과 결정 기록 한 줄에 한정한다
> 개정: 2026-09-23 v2.4. FR-24의 위치당 영역 상한을 81칸에서 2,500칸으로 올렸다(MSG-600, 사용자 결정).
> 청계천 경계 기술검증(`docs/reports/2026-09-13-event-tour-additive-poc.md`)에서 하천이나 거리처럼
> 가늘고 긴 행사장은 면적이 작아도 걸치는 칸이 많아 81칸으로는 신청 자체가 안 되는 것이 확인됐다.
> 새 값은 행사 위치 시드가 이미 쓰는 상한과 같은 값이라 상한이 한 벌이 된다. 개정 범위는 FR-24 한
> 행, 비기능 요구 두 칸, 결정 기록 한 줄, 개정 영향 파일 표에 한정한다
...
| FR-6 | 계정이 없는 행사 운영자는 비로그인 공개 폼으로 계정 발급을 요청할 수 있다(기관명, 담당자명, 연락처, 공식 이메일, 예정 행사명, 요청 내용). 관리자는 요청 큐에서 검토해 승인(계정 생성과 초기 비밀번호 발송)하거나 사유를 적어 반려한다. 반려하면 요청자의 공식 이메일로 반려 사유를 담은 안내 메일이 즉시 발송된다. 메일 본문은 필맵 이름과 색을 입힌 서비스 전용 서식이고 다시 신청하는 길을 안내한다. 발송 실패는 반려를 뒤집지 않고 관리자 응답에 발송 실패로만 드러난다(v2.3, MSG-575. 그전까지는 반려 통보가 수기였다, 피그마 댓글 #108) | Must |
...
| FR-8 | 행사 위치는 주소가 아니라 지도 위 격자 사각형 영역으로 지정한다. 신청 하나에 위치 여러 개, 위치 하나에 사각형 여러 개를 담을 수 있다. 이벤트 참여형은 예외로 대표 위치 1곳만 받는다(ver 14 [4C] 시안, MSG-502 결정 D-2, v2.2). 형식은 기존 행사 시드의 areaRects[^2]와 같다. 위치에 이름을 붙이지 않고, 순번과 지역 라벨(격자 표시명 재료)로만 식별한다(피그마 댓글 #102) | Must |
| FR-24 | 위치 하나가 쓸 수 있는 격자는 사각형들을 합쳐(겹침은 한 번만 센다) 최대 2,500칸이다. 50×50 격자, 5km×5km 상당이고 행사 위치 시드의 위치당 상한과 같은 값이다. 초과는 제출에서 막고 사유를 돌려준다. 사각형 하나가 단독으로 2,500칸을 넘거나 위치 하나의 사각형 개수가 2,500개를 넘어도 같은 거절이다. 반려본을 고쳐 다시 낼 때도 같은 상한이 적용된다(v2.4, 2026-09-23 사용자 결정. 그전까지는 81칸(9×9 유래, 피그마 댓글 #106·#109 "1km로 합의")이었다. 번복 근거는 §8 결정 기록) | Must |
...
| 이벤트 참여형 | 이름(참여로 생기는 실체는 별도 명칭 없이 부모 이벤트의 행사 위치로 흡수, v2.2 확정), 운영 주체, 공개 기간, 참여 방식, 소개, 커버 이미지. 입력 전에 시·도 칩과 승인 이벤트 목록 모달([3-1] 신규)에서 부모 이벤트를 고른다 |
...
| 데이터 정합 | 승인 시 생성되는 행사 데이터는 기존 DDL 제약(회차 미혼합 복합 FK, 격자 단일 귀속, 대표 격자 소속)을 그대로 통과해야 한다. 승인 반영과 신청 상태 갱신은 함께 성공하거나 함께 실패한다. 위치당 영역 상한은 신청 접수와 행사 위치 시드가 같은 값을 쓴다(v2.4). 승인이 신청을 행사 위치나 미션으로 전개할 때 그 결과가 시드 상한을 넘는 일이 생기지 않아야 하고, 상한 값은 한 곳에서만 정의된다 |
...
| 성능 | 콘솔과 심사는 내부 소수 사용자 대상이라 별도 성능 목표를 두지 않는다. 일반 사용자 노출 경로는 기존 API 재사용이라 영향이 없다. 상한이 2,500칸으로 오르면서 신청 하나의 전개 칸 수 상한은 위치 20개 × 2,500칸 = 50,000칸이 됐다(v2.4). 제출 검증, 신청 상세, 심사 상세, 승인 전개는 이 규모의 신청에서도 요청 한 번에 성립해야 한다. 승인된 위치의 격자 수가 늘어도 지도 홈의 미션 칩, 행사 칩, 뷰포트 조회의 응답 시간 목표는 바뀌지 않는다. 다만 이 값은 시드 위치가 이미 허용하던 규모라 새 부하 유형이 아니고, 별도 부하 검증 목표를 두지 않는다 |
...
### v2.4 개정 영향 파일 (2026-09-23, 영역 상한 81칸 → 2,500칸)

2026-09-23 리서치로 확인한 현재 코드 기준이다. 스키마 변경은 없다. DB에는 칸 수 제약이 없고
상한은 서버 상수 하나가 판정한다.

| 파일 | 변경 | Owner |
|------|------|-------|
| `event/submission/service/EventSubmissionServiceImpl.java` | 수정. 위치당 상한 상수(현재 81)를 시드 상한과 같은 값으로. 사각형 개수 상한이 같은 상수에 묶여 있어 함께 오른다 | B |
| `event/seed/EventSeeder.java` | 수정. 위치당 2,500칸 상수를 공용 위치로 승격해 신청 검증과 한 정의를 공유한다(같은 값의 사본 금지. 격자 인덱스 상한을 승격한 MSG-498 선례) | B |
| `event/exception/EventErrorCode.java` | 수정. 13432 메시지의 "최대 81칸" 문구 | B |
| `event/submission/dto/EventSubmissionLocationRequestDto.java`, `EventSubmissionLocationResponseDto.java` | 수정. Swagger 설명의 81칸 표기 | B |
| `src/test/java/.../EventSubmissionValidationTest.java` | 수정. 경계값 4건(합산 81/82, 단일 사각형 초과, 사각형 81개 초과, 이벤트 위치 상한)을 새 값으로 | B |
| `docs/spec/MSG-498.md`, `docs/spec/MSG-500.md` | 수정. D-7 상한 서술과 MSG-500 인계 조건의 "신청 상한 81칸·시드 상한 2,500칸이 다른 값" 문장(같은 값이 되면서 재확인 조건이 사라진다) | B |
| `docs/srs.md` FR-EVENT-13·FR-EVENT-17 | 수정. "최대 격자 81칸" 문구. srs-writer 몫 | B |
| FE 위저드 스텝 3 (MSG-547 산출물) | 수정. "n/81" 진행 바와 초과 차단 값. 한 변 10칸 경고는 유지 여부를 FE가 정한다(§8) | FE |

`RepresentativeGridResolver`는 셀 수 2,500을 전제로 안전 논증을 세워 두었으므로 변경이
없다. 승인 시 미션으로 전개하는 경로(`MissionRegistrationService`)에는 칸 수 상한이 따로
없고 시드 미션도 같은 규모를 허용하므로 변경이 없다.

## 8. 미해결 질문

2026-08-28 개정에서 초안의 6건 중 3건이 ver 14 시안과 피그마 댓글로 해소됐고, 같은 날
작성자(강정민) 확인으로 2건이 더 닫혔다. 2026-08-29 v2.2에서 노출 방식과 참여형 세부 1건이
사용자 확정으로 추가로 닫혀, 남은 미확정은 1건(승인 후 일정 수정 정책, 유보 확정)이었다.
2026-09-23 v2.4에서 영역 상한 번복에 따른 디자인 주석 갱신 1건이 새로 열려 미확정은 2건이다.

- [x] ~~승인 행사 노출 방식과 이벤트 참여형의 세부~~ → **두 갈래 모두 확정** (2026-08-29
  사용자(정민) 확정, v2.2): ① **참여형 하위 실체는 별도 명칭 없이 "행사 위치"로 흡수한다.**
  참여 신청이 승인되면 부모 이벤트 아래 행사 위치(`event_locations`)로 반영되고, 이름, 운영
  주체, 소개, 공개 기간, 참여 방식 같은 추가 속성은 위치에 붙는 부가 정보다. 새 용어를
  만들지 않는다(MSG-503이 미정으로 남긴 하위 실체 명칭이 이렇게 닫혔다. glossary "이벤트"
  항목의 미정 표기 갱신은 별도 회부). 저장과 반영의 몫은 갈린다: MSG-502는 이 속성들을 신청
  행에 저장하는 데까지이고, 승인 반영 시 `event_locations` 확장 컬럼(별도 마이그레이션)과
  신청에서 위치로의 복사 매핑, 사용자 조회 노출 계약은 MSG-500이 정의한다(지금
  `event_locations`에는 이름 컬럼뿐이라 이 확장 없이는 운영 주체, 소개, 공개 기간, 참여
  방식이 승인 후 노출될 경로가 없다).
  ② **지역축제와 팝업스토어 승인분은 기존 미션 칩으로 편입한다.** 승인된
  지역축제 신청은 축제 미션(`MissionType.EVENT`), 팝업스토어 신청은 팝업 미션
  (`MissionType.POPUP`)으로 등재되어 지도 홈의 기존 축제·팝업 칩 목록에 추가된다. 이벤트
  공간(구 행사방)이나 새 칩, 규모별 분기는 만들지 않고, 이벤트 공간은 이벤트 카테고리
  (지역축제와 팝업 제외) 전용으로 유지된다(MSG-503 용어 체계와 정합). 구현은 MSG-500
  approve 몫이고, "확정되면 SRS FR-EVENT-01의 등재 기준 서술을 함께 고친다"는 후속도
  MSG-500 레인 몫이다. 이 확정으로 MSG-502(참여 신청)와 MSG-500(approve)의 스펙 착수
  게이트가 풀렸다
...
- [x] ~~위치당 영역 크기 상한~~ → ~~위치당 총 81칸(9×9 유래)으로 확정~~ (피그마 댓글 #106·#109
  ...
  → **2,500칸으로 번복** (2026-09-23 사용자(정민) 결정, v2.4). 발단은 청계천 경계 기술검증이다.
  청계천 중심선에 편측 20m 경계를 만들면 면적은 33ha(격자 33칸 상당)인데 걸치는 격자는 135칸이고,
  편측 40m면 167칸이다. 81칸 합의는 위치 하나가 대략 1km 사방의 덩어리라는 상정 위에 있었는데,
  하천이나 거리 축제처럼 가늘고 긴 행사장은 그 상정을 벗어나 면적이 작아도 신청이 막힌다.
  새 값을 2,500으로 고른 이유는 행사 위치 시드가 이미 위치당 2,500칸(5km×5km 상당)을
  허용하고 있고 대표 격자 계산기도 그 값을 전제로 안전 논증을 세워 두어서, 신청과 시드의 상한이
  한 벌로 합쳐지고 새로 검증할 규모가 생기지 않기 때문이다. 검토한 다른 안은 둘이다. 하나는 값을
  조금만 올리는 것(예: 300칸)인데 상한이 두 벌로 남고 다음 선형 행사장에서 같은 일이 반복된다.
  다른 하나는 사각형 대신 경계 도형 입력을 받고 서버가 칸을 전개하는 것인데, 이건 상한 문제가
  아니라 입력 계약 문제라 별도 PRD(행사 정밀 경계)로 넘긴다. 이 개정만으로 청계천을 신청하려면
  운영자가 굽은 선을 따라 사각형을 여러 개 그려야 하는 불편은 남는다. 값이 오르면서 심사가 실질
  방어선이 된다. 5km 사방을 한 위치로 신청하는 것은 형식 검증을 통과하므로 관리자가 규모를 보고
  반려하는 것이 전제다(MSG-547이 "한 변 10칸 경고, 확정은 허용, 관리자 심사가 최종 판단"으로
  같은 구도를 이미 택했다)
- [ ] **디자인 주석의 상한 값 갱신** (v2.4 신설, 디자인 미확인): v2 [행사 운영자 5]의 "9/81" 카운터와
  영역 상한 주석(`15525:8840`)이 81을 못 박고 있다. 이번 개정에서 피그마를 다시 열어 확인하지
  않았으므로(월 조회 한도), 주석을 2,500으로 고칠지 카운터 표기를 바꿀지는 디자인과 FE(MSG-547
  담당) 몫이다. 서버는 값만 바꾸고 화면 표기 형식은 정하지 않는다. FE의 "한 변 10칸 경고"는 값이
  올라도 유지를 권한다. 큰 사각형을 막는 것이 아니라 심사자에게 규모를 알리는 장치라서다
- [x] ~~계정 발급 요청의 접수 방식~~ → **API 접수로 확정** (v2 [행사 운영자 1-1] 공개 폼 +
  [관리자 1-1] 요청 큐 + 서버 재료의 `POST /api/org-account-requests`). 반려 통보만 수기(#108). FR-6으로 등재
- [x] ~~반려 통보 수기(#108)~~ → **반려 안내 메일 자동 발송으로 번복** (2026-09-05 사용자 지시, MSG-575).
  승인은 메일이 자동으로 나가는데 반려만 수기라 관리자가 매번 따로 써야 했고, 잊으면 요청자가
  결과를 모르는 채 남았다. 관리자가 적은 반려 사유를 그대로 싣고 필맵 전용 서식(HTML, 평문 대체본
  동봉)으로 보낸다. 아이디 변경 요청 반려(MSG-500)와 기존 메일 4종의 서식 전환은 범위 밖이다.
  SRS FR-AUTH-18
