...
Owner 판정: 조회 축이 user_grids와 videos(커버)라 구현 주도는 Owner B다. 행정동 귀속은 grids.region_code를 같은 쿼리에서 읽는 기존 B 내부 선례(`/api/collections/videos`, MSG-167)를 따르므로 새 경계 설계는 필요 없다. 다만 `UserGridQueryService`는 계약 인터페이스라 시그니처 확장에 상대 팀원 확인이 필요하다(스펙에서 공동으로 분류).

## 9. 미해결 질문

- [ ] 카드 커버는 도감 갤러리와 같은 user_grids 커버를 쓰는 것으로 제안한다(격자의 첫 내 영상이 유지되고, 삭제나 블라인드로 사라지면 재선정되며, 블라인드 해제 시 원복은 없다. 재선정 대상이 없으면 커버 없이 카드만 남는다). 디자인이 격자 대표 영상(전역 기준)이나 최신 영상을 요구할 가능성만 확인 필요.
