...
| FR-9 | 응답은 묶음 목록과 현재 행정동을 함께 담는 하나의 형태로 통일한다. 내 격자 기준으로 요청하거나 구, 시 단위로 요청하면 현재 행정동만 빈 값이고 형태는 같다. 기준에 따라 응답 모양이 갈리지 않는다 | Must |
...
친구 기준 주소(`GET /api/friends/{userId}/grids/aggregation`)는 이번 범위 밖이고 그대로 둔다.
...
| `src/main/java/com/msg/fillmap/friend/` | 친구 기준 집계는 기존 응답 타입을 재사용 중이라, 응답 타입이 바뀌면 그 재사용 지점을 확인한다. 친구 기준 계약 자체는 바꾸지 않는다 | A |
