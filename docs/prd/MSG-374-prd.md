...
| 파일 | 변경 | Owner |
|------|------|-------|
| `src/main/java/com/msg/fillmap/grid/controller/GridController.java` | 기존 `GET /api/grids/aggregation`에 축 파라미터를 넓혀 전역 축을 받는다(2026-08-11 확정. 파라미터 이름, 기본값, 기존 호출 호환은 스펙 확정) | A |
...
엔드포인트 형태(기존 경로에 축 파라미터 확장)와 현재 행정동 이름 조립(서버가 한 줄로)은 2026-08-11에 확정해 본문에 반영했다. 축 파라미터 확장은 FE가 부를 주소가 그대로라는 뜻이므로 스펙 확정 시 파라미터 계약만 FE에 통보하면 된다.
