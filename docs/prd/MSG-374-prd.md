...
| FR-12 | 묶음의 격자 수와 영상 수는 화면을 덮는 격자 범위 안의 격자를 센다. 행정동이 화면 경계에 걸치면 그 동의 범위 밖 격자는 빠지고, 지도를 움직이면 같은 동의 숫자도 달라진다. 범위 산출은 기존 뷰포트 조회 3종이 함께 쓰는 규칙 그대로이며, 격자 축이 위경도 축과 평행하지 않아 가장자리에서 화면 밖 셀이 소량 포함되는 것도 그대로 받아들인다(MSG-347에서 무해로 판정한 동작). 정확한 포함 판정을 새로 넣지 않는다 | Must |
