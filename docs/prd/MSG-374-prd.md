...
| FR-6 | 친구 기준 집계가 내보내는 응답은 필드 하나까지 지금 그대로다. 영상 수는 친구 도감이 요구한 적이 없어 담지 않는다. 그러려면 두 조회가 함께 쓰는 **응답 타입**을 나눠야 한다. 친구 조회는 지금 응답 타입을 계속 쓰고 영상 수가 붙는 응답 타입은 지도 홈 조회에만 둔다. 한 응답 타입에 필드를 더하면 이 티켓과 무관한 친구 도감의 계약까지 바뀐다. 서버 안에서 두 조회가 계산 결과를 담는 내부 타입을 공유할지는 구현 판단이라 스펙이 정한다 | Must |
...
classDiagram
    ...
    class 묶음항목_기존 {
        %% 친구 집계가 계속 쓰는 응답 타입(RegionAggregateResponseDto). 필드 불변
    }
    class 현재행정동 {
        +String regionCode
        +String name
