...
| FR-12 | 동 단위 묶음의 격자 수와 영상 수는 같은 행정동을 기존 탐색 조회(`GET /api/regions/{regionCode}/grids`, MSG-238)로 물었을 때의 헤더 카운트와 일치한다. 두 조회가 같은 공개 게이트 정의를 쓰기 때문이며, 사용자가 마커를 눌러 패널을 열었을 때 숫자가 달라 보이면 안 된다 | Must |
