...
첫째, **서버가 이름을 아예 안 보낸다.** 미션 응답의 스팟 항목은 `gridId`와 좌표와 순번뿐이다.
그래서 화면은 스팟마다 좌표를 역지오코딩해 동 이름을 얻고 있는데, 그 조회는 좌표를 품는 행정동이
없으면 빈 값을 돌려준다. 해파랑길 1코스 7곳이 전부 "이름 없는 경유 지점"이던 것이 이 경우다 —
바다에 걸친 지점이라 품는 행정동이 없다. 내륙 코스에서는 동 이름이 나오므로, 지금 화면은 코스에
따라 되기도 하고 안 되기도 하는 상태다.

격자 쪽으로 이름을 얻는 길은 아예 막혀 있다. 코스 스팟 972칸 중 **951칸(98%)은 `grids` 테이블에
행 자체가 없다.** 격자 행은 누군가 영상을 올릴 때 비로소 만들어지는 구조라, 아무도 안 다녀간 경유
지점은 조회할 대상이 존재하지 않는다.
