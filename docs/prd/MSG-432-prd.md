...
> 티켓: MSG-432 · 작성일: 2026-09-23 · 작성: FE 하네스(성민 위임)
> 상태: 검토됨 (2026-09-24 성민 확정 — 8절 질문 4건 해소)
...
| targetType | 앱 목적지 | 비고 |
|---|---|---|
| `VIDEO` | 영상 재생 화면 `/video/{videoId}` | MODERATION(가려짐)도 같은 곳. 재생 API가 BLINDED 영상을 소유자에게는 통과시키고 재생 URL만 비우므로 앱 재생 화면이 "가려짐" 사유를 그대로 보여 준다 (Q3 확정) |
| `GRID` | 지도 홈 `/home?gridId={gridId}` (기존 검색 진입 파라미터 재사용 — 해당 격자로 이동·하이라이트) | 셀 상세 시트는 열지 않는다 (Q4 확정 — 시트는 내 점령 격자만 열리는 게이트 뒤라 핫구역 격자에서는 열려도 막힌다) |
...
| `USER` | 지도 홈 `/home` (친구 화면이 생길 때까지) | **모바일에 친구 화면이 없다**(바텀 탭 "친구"는 MSG-556에서 AI 추천으로 교체). 서버는 `USER`를 기록해 두고 웹·후속 화면이 쓴다 (Q1 확정) |
| 없음 / 미지 | 지도 홈 `/home` | REMIND·WEEKLY·기존 알림. REMIND를 업로드 화면으로 보내는 안은 기각 — 대상 종류를 화면 이름으로 남용하지 않는다 (Q2 확정) |
...
## 8. 질문과 결정 (2026-09-24 성민 확정)

- [x] **Q1. FRIEND 알림의 목적지** — 모바일에 친구 화면이 없다. (a) 친구 화면이 생길 때까지 지도 홈(제안), (b) 상대 프로필 미리보기(`USER` 대상이 있으니 가능하지만 화면이 없음), (c) FRIEND는 이번 계약에서 대상 없음으로 두고 친구 화면 티켓에서 다시 정한다. → **결정 (a)**: 서버는 `USER`를 기록해 두고(웹·후속 화면이 쓴다) 앱은 홈으로.
- [x] **Q2. REMIND(스트릭) 목적지** — 대상 없음이라 홈이 기본. "영상 하나만 올리면 이어져요"라는 문구상 **업로드 시작 화면**(`/upload`)이 더 맞을 수 있다. 그러려면 대상 종류 `UPLOAD`(식별자 없음)를 두거나 FE가 카테고리로 예외 처리해야 한다. → **결정**: 홈 유지(대상 종류를 "화면 이름"으로 남용하지 않는다). 지표를 보고 바꾼다.
- [x] **Q3. MODERATION(가려짐) → 영상 화면** — 소유자가 가려진 자기 영상 화면에 들어갈 수 있는지 서버 재생 API 정책 확인 필요. 못 들어가면 영상 화면의 404·차단 안내가 뜨는데, "왜 가려졌는지"를 보러 온 사용자에게 적절한 안내인지. → **결정**: `VIDEO`. `VideoServiceImpl.getVideoPlayback`이 BLINDED를 소유자에게 통과시키고(타인은 404) 재생 URL만 null로 내리며, 앱 `video-playback.ts`가 BLINDED를 별도 사유 문구로 처리한다(코드 확인 2026-09-24).
- [x] **Q4. GRID 진입 깊이** — `/home?gridId=`는 격자로 이동·하이라이트까지다(검색 진입 재사용). 핫구역 알림은 셀 상세 시트(내 점령·핫구역 정보)까지 열어 주는 편이 자연스럽다. FE 작업량 차이가 작으면 시트까지 여는 것을 제안. → **결정**: 이동·하이라이트까지만. 시트는 새 파라미터와 `canOpenDetail` 게이트(내 점령 격자·테마 격자만) 우회가 필요해 작업량 차이가 작지 않고, 핫구역 격자는 대개 내 격자가 아니라 우회해도 의미가 없다.
