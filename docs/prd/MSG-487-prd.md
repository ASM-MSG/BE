...
작성 시점에 네 건이 있었다. 한 건은 2026-08-26 성민이 직접 확정했고, 두 건은 같은 날 위임받은
스펙이 값을 정해 해소됐다. 남은 것은 결과 부족 문구의 디자인 확정 한 건이다.
...
- [x] **축소 신호의 화면 소비 (스펙 몫, 2026-08-26 성민 위임).** 정규화 뒤에도 부산처럼 큰
      지역을 언급하면 서버는 축소 신호(ZOOM_OUT)를 계속 만든다. 1~2km 화면은 그 지역의
      일부만 담기 때문이다. 위임받은 스펙(docs/spec/MSG-487.md 결정 1)이 같은 날 확정했다:
      화면이 이 신호를 무시한다(안내 없음). "더 넓게 볼까요"는 넓혀도 다음 실행에서 도로
      좁아지니 자기모순이라 폐기하고, 서버의 신호 생성 코드는 지표 실측 재료로 불변이다.
      SRS FR-ROUTE-14의 축소 안내 조항도 같은 날 이 결정으로 정정됐다.
...
- [x] **정규화 대역의 정의.** 위임받은 스펙(docs/spec/MSG-487.md 결정 3)이 같은 날 정했다:
      실행 시점에 항상 목표 축척(뷰포트 짧은 변의 지상 길이 약 2km)으로 맞추며, 1~2km는
      규칙이 아니라 지도의 이산 줌 단계 때문에 실제 값이 떨어지는 허용 범위다. 대역 안이면
      유지하는 안은 같은 자리 같은 문장의 동일 범위 보장(FR-ROUTE-15)이 깨져 기각됐다
      (Codex 리뷰 적발). FR-6과 SRS FR-ROUTE-15도 이 정의로 정정됐다.
