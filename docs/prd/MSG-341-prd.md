...
- **목표**:
  ...
  - 명명 규칙의 정본(`zone-naming.json` 픽스처[^1])은 하나로 유지되고, 유일한 계산 구현(서버)이 그 정본으로 검증된다.
- **비목표(스코프 제외)**:
...
| 데이터 정합 | 명명 규칙의 단일 정본은 계속 `zone-naming.json` 하나다. 서버 테스트가 이 픽스처를 직접 소비해, 서버 계산이 정본 규칙에서 어긋날 수 없게 한다 |
