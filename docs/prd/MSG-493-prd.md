...
| FR-3 | 거리 상한은 3km다(2026-08-27 정민 확정). 실측된 코스 경유 지점 최댓값 2,189m는 안에 들고, 경계에서 3km를 넘는 바다 한가운데 격자는 종전처럼 이름이 빈다. 지도 격자망은 아무 바다나 눌리므로 상한이 없으면 수십 km 떨어진 동 이름이 붙는다 | Must |
| FR-4 | 행정동에 속하는 격자의 응답은 바뀌지 않는다. 폴백은 품는 행정동이 없을 때만 돈다 | Must |
| FR-5 | 이 폴백은 표시 이름에만 관여한다. 수집률, 탐험률, 동 단위 영상 조회의 귀속은 변하지 않는다 | Must |
| FR-6 | 명소 이름도 구역 표시명도 없이 최근접 행정동으로 적재된 경유 지점(MSG-492 사다리 ④) 중 경계 3km 안인 곳에 한해, 목록과 격자 화면이 같은 행정동을 가리킨다. 3km 밖 지점은 목록에만 이름이 있고 격자 화면은 비는데, 실측된 경유 지점 최댓값이 2,189m라 현재 데이터에는 그런 지점이 없다. 최근접 판정 규칙이 같고 결정적이라 행정동이 갈리지 않는다. 문자열은 다르다 — 목록은 마지막 토큰("다대2동"), 격자 화면은 전체 경로 재료다. 명소나 구역 이름이 붙은 경유 지점은 목록이 그 이름을 우선하므로 이 요구의 대상이 아니다 | Must |
...
신규 타입 없음. MSG-492가 만든 계약 `RegionQueryService.resolveNearestByPoint`에 거리
상한 오버로드가 하나 늘고, 소비 지점이 하나(단일 격자 조회) 늘어난다.

## 7. 변경 파일 목록

| 파일 | 변경 | Owner |
|------|------|-------|
| `grid/service/impl/GridQueryServiceImpl.java` | 단일 격자 조회의 행정동 판정에 최근접 폴백 (품는 행정동 없을 때만, 상한 3km) | A |
| `region/service/RegionQueryService.java` | 거리 상한 오버로드 추가 (3km 확정으로 발생) | A |
| `region/service/impl/RegionQueryServiceImpl.java` | 오버로드 구현 | A |
| `region/repository/RegionRepository.java` | 상한 필터가 있는 최근접 쿼리 추가 | A |
...
- **표시 이름은 가장 가까운 행정동으로 채운다** (2026-08-27 정민). 화면 이름과 집계
  ...
  화면이 일관된다
- **표시 폴백의 거리 상한은 3km다** (2026-08-27 정민). 처음에는 MSG-492 경유 지점과 같은
  "상한 없음"으로 갔는데, 그쪽은 대상이 검증된 972칸(최대 2,189m)이고 이쪽은 지도의 아무
  격자나 대상이라 전제가 다르다. 바다 한가운데를 누르면 90km 떨어진 동 이름이 붙는 것을
  실측으로 확인해 상한을 되살렸다. 코스 시더의 상한 없음(MSG-492 D-5)은 그대로다
