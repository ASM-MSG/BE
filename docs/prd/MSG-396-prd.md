...
| FR-14 | 같은 화면에 놓인 내 영상 카드와 전역 영상 카드가 같은 축의 시각을 보여준다. 둘 다 업로드 시각이고, 하단의 "마지막 업로드"와도 같은 축이다 | Must |
...
| `src/main/java/com/msg/fillmap/video/dto/GridGlobalVideoResponseDto.java` | 수정: 카드 표시 시각을 업로드 시각 축으로 (FR-14). MSG-390이 재사용하는 타입이라 파급 확인 필요 | B |
...
- **전역 영상 카드의 표시 시각도 업로드 시각으로 맞춘다.** 지금은 내 영상 카드가 업로드 시각을,
  전역 영상 카드가 촬영 시각을 보여준다. 같은 화면에 나란히 놓인 두 카드가 다른 축을 쓰는 상태라
  "어제"와 "8월 9일"의 뜻이 카드마다 다르다. 화면에 그런 구분이 없으므로 축을 하나로 맞춘다
  (FR-14)
...
MSG-390이 미션 영상 목록에서 `GridGlobalVideoResponseDto`를 재사용하기로 했고 정렬을 촬영 시각
최신순으로 확정했다. 그 근거는 미션 기간 판정이 촬영 시각을 쓰고 카드에 표시되는 시각도 촬영
시각이라는 것이었다. 위 셋째 항목이 그 두 번째 근거를 바꾼다.

미션 쪽 정렬을 함께 바꿀지는 이 문서가 정하지 않는다. 미션에는 기간 조건이 있어 촬영 시각을 쓸
독립적인 이유가 남아 있고, MSG-390은 스펙까지 마감한 상태다. 다만 **카드 표시 시각이 업로드
시각으로 바뀌면 MSG-390 스펙의 "표시와 정렬을 같은 축으로 맞춘다"는 근거가 미션 쪽에서는 더
이상 성립하지 않는다.** 이 사실을 MSG-390 스펙에 반영해야 하고, 구현 순서상 먼저 착수하는 쪽이
공유 DTO를 건드리므로 두 티켓의 순서를 정할 때 이 점을 본다.
