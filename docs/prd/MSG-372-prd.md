...
이 절의 FR-1부터 FR-7은 전역 요구사항 명세(`docs/srs.md`)에 **FR-MAP-09** 하나로 등재돼 있다.
...
| FR-3 | 시간대 구간은 KST[^3] 기준이다. 저장 시각이 UTC이므로 변환 후 시(0~23)를 뽑는다. 기준 시각은 업로드 시각[^4]이다. 단 dev의 psql 직접 시딩분은 KST 벽시계로 저장돼 있어(MSG-376 잔재) 그 보정 티켓 MSG-379가 끝나기 전까지 dev 차트에서 해당 행만 9시간 밀려 보인다. prod와 dev의 앱 경로 저장은 UTC 실측 확인(MSG-376)이라 실사용 데이터에는 문제가 없다 | Must |
...
| 분류 | 요구사항 |
|------|----------|
| 성능 | 칩 맥락의 격자 선택마다 1콜이 추가된다. 지도 화면 계열 조회이므로 같은 SLO[^5](p95 300ms 미만)를 지킨다. 이 목표는 SRS NFR-PERF-01(지도 뷰포트 조회)의 응답 시간 목표를 이 조회까지 넓혀 적용하는 것이며 SRS 갱신을 동반한다(측정 방법은 NFR-PERF-02의 초당 요청 수 기준 그대로). 격자 하나로 거르는 조회라 기존 격자 축 인덱스를 탈 수 있는지 스펙에서 확인한다 |
