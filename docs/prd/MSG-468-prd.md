...
- **목표**:
  ...
  - 지역을 말하지 않았거나 화면 안의 지역을 말한 요청은 지금과 완전히 같게 동작한다.
...
- **비목표(스코프 제외)**:
  ...
  - 언급 지역의 결과를 미리 가져와 두 지역을 함께 보여주는 일은 하지 않는다. 지도를 옮긴 뒤의
    추천은 새 요청이다.
  - 지역 별칭 사전을 만들지 않는다. 서버가 가진 행정구역 이름과 맞는 범위까지만 판정한다.

## 3. 기능 요구사항

| ID | 요구사항 | 우선순위 | SRS |
|----|----------|----------|-----|
| FR-1 | 문장에서 해석된 지역 이름이 서버가 아는 행정구역과 맞고 그 구역이 요청 뷰포트와 겹치지 않으면, 응답에 언급 지역의 이름과 중심 좌표가 함께 실린다. 추천 지점 목록은 지금처럼 뷰포트 기준으로 그대로 온다 | Must | FR-ROUTE-14 |
| FR-2 | 지역 언급이 없는 요청, 그리고 언급 지역이 뷰포트에 걸치는 요청은 이 신호가 비어 있고 나머지 응답이 지금과 같다 | Must | FR-ROUTE-14 |
| FR-3 | 해석된 지역 이름을 행정구역과 맞추지 못하면(오타, 해외 지명, 서버에 없는 통칭) 신호 없이 지금과 같은 응답을 내린다. 판정이 안 된다고 추천이 실패로 바뀌지 않는다 | Must | FR-ROUTE-14 |
| FR-4 | 화면은 신호가 있으면 추천 결과와 함께 언급 지역을 짚어 주는 안내를 보여주고, 사용자가 수락하면 지도를 그 지역 중심으로 옮긴다 | Must (FE 몫) | FR-ROUTE-14 |
...
| 데이터 정합 | 응답에 싣는 지역 이름과 좌표의 근거는 서버가 보유한 행정구역 데이터뿐이다. 자연어 해석이 만들어 낸 지명이나 좌표를 검증 없이 그대로 싣지 않는다(NFR-SEC-08과 같은 정신) |
...
지역 언급이 없거나 언급 지역이 뷰포트와 겹치면 판정 단계가 신호 없음으로 끝나고 응답은 지금과
같다. 판정 실패(FR-3)도 같은 길이다.
...
- [ ] 같은 이름이 여러 행정구역에 걸릴 때(전국에 "중구"가 여럿이다) 판정 규칙. 하나라도
      뷰포트와 겹치면 신호를 싣지 않는 쪽이 안전해 보이는데, 전부 밖일 때 어느 중심을 보낼지
      또는 아예 싣지 않을지는 스펙 전에 정한다.
- [ ] "서면"이나 "홍대"처럼 행정구역이 아닌 통칭을 zones 데이터로도 맞출지. 이번 범위에서 빼면
      그런 문장은 지금과 같은 무신호로 남아 해가 늘지는 않는다.
- [ ] 지도를 옮긴 뒤 재추천을 자동으로 보낼지. 자동이면 10초 요청 제한(FR-ROUTE-12)과
      부딪친다. 사용자가 버튼을 다시 누르는 수동을 기본으로 제안한다.
