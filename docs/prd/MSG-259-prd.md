...
- **목표**:
  - 주요 상권·유명지 30~50개가 `zones`에 시딩되어, 그 사각형 안 격자의 표시명이
    행정동 이름에서 `"서면 A-14"` 형태로 바뀐다. 초기 물량은 서울·부산 중심이되 지역 제한은 없다.
...
- **비목표(스코프 제외)**:
  ...
  - ~~**전국 확대** — 서울·부산만~~ → **지역 못박기 폐지 (2026-07-31)**: 유명하거나 특별한 통칭이면
    지역 무관하게 등재한다 (예: 경주 황리단길). 서울·부산은 공공데이터 **일괄 변환**을 돌린 범위일 뿐이고,
    그 외 지역은 통칭 단위 수동 작도로 추가한다. 타 시도 공공데이터 일괄 재실행은 여전히 후속.
...
- [x] **통명화 기준** — **확정(2026-07-30)**: 유명 통칭 목록 선정 → 같은 시/도 안에서만 합집합
      ...
      **2쌍**만 남아 경계 정수 조정으로 분리(FR-7 — 확정 당시 추정 3쌍 중 강남↔압구정은 드라이런에서
      부분문자열 과매칭 허상으로 판명). priority 서열 기준은 만들지 않는다
