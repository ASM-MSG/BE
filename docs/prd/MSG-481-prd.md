...
- **목적**: 보고 있는 화면 주변을 검색할 때 화면과 결과를 일치시킨다. 지금은 부산 서면을 보면서 "서면"을
  쳐도 순천이 나와, 눈앞의 장소조차 검색으로 짚지 못한다.
  범위를 좁혀 적는다. 이 기능이 맡는 것은 **화면 주변 탐색**이고, 멀리 떨어진 목적지로 지도를 옮기는
  흐름까지 책임지지 않는다. 후자는 사용자가 지도를 옮기고 다시 검색하는 것이 지금의 방식이며, 그 경계에서
  생기는 한계는 §9에 실측과 함께 적었다. 두 흐름을 한 요청으로 모두 만족시키려면 외부 호출을 항상 두 번
  해야 해서 이번에는 택하지 않았다.
- **목표**
  ...
  - 좌표를 보낸 검색은 반경 20km 안으로 좁혀진다. 근처에 결과가 하나라도 있으면 그 밖의 장소는 이번
    응답에 들어오지 않는다. 이것이 실제 동작이고, 되돌아가는 조건은 근처가 0건일 때뿐이다.
- **비목표(스코프 제외)**
  - 거리순 정렬. 반경 안에서도 순서는 공급자 정확도순 그대로다.
  - 좌표를 보내지 않은 검색의 동작 변경. 그쪽은 반경이 걸리지 않는다.
...
| ID | 요구사항 | 우선순위 |
|----|----------|----------|
| FR-1 | 사용자는 검색어와 함께 지금 보고 있는 지도의 중심 좌표를 보낼 수 있고, 그 좌표에서 20km 안에 있는 장소가 결과에 온다. 반경 밖 장소는 FR-2 가 걸리지 않는 한 응답에 들어오지 않는다 | Must |
