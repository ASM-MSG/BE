...
- **목표**:
  ...
  - 동의 5항목이 저장된다. 철회가 없는 필수 3항목은 동의 시각이 증빙으로 남고, 철회할 수
    있는 위치기반과 마케팅은 현재 상태와 마지막 변경 시각이 남는다 (FR-4)
...
| FR-4 | 철회가 없는 필수 3항목(만 14세, 서비스 이용약관, 개인정보)은 동의 시각이 저장되어 증빙으로 남는다. 철회할 수 있는 항목(위치기반서비스, 마케팅 수신)은 현재 상태와 마지막으로 동의하거나 철회한 시각을 보관한다 (FR-USER-14가 위치 동의에 확정한 방식과 동일. Codex 리뷰 지적으로 2026-08-19 문구 정밀화 — 철회 항목은 철회 뒤 원 동의 시각이 남지 않는다) | Must |
| FR-5 | 위치기반서비스 항목은 기존 위치 동의 저장(FR-USER-14, `location_consent`)과 한 값으로 관리된다. 가입 동의 제출이 위치 동의를 켜고, 이후 프로필 편집의 위치 동의 변경과 서로 모순되지 않는다 | Must |
