...
| FR-5 | 무효한 인가 코드(만료, 재사용, redirect URI 불일치)로는 로그인할 수 없다. 교환 실패는 401 도메인 에러 하나로 응답하고, 카카오의 상세 사유(KOE 코드[^5])는 서버 로그에만 남긴다. | Must |
| FR-6 | 카카오 토큰 응답에 ID 토큰이 없으면(인가 요청에 `scope=openid` 누락) FR-5와 같은 401 에러로 응답한다. | Must |
| FR-7 | 카카오 서버 무응답이나 5xx 시 제공자 오류를 뜻하는 5xx 대역 에러로 응답한다. 타임아웃 없이 무한 대기하지 않는다. | Must |

## 4. 비기능 요구사항

| 분류 | 요구사항 |
|------|----------|
| 보안 | REST API 키(그리고 도입 시 client_secret[^6])는 서버 환경변수로만 보관한다. 인가 코드, ID 토큰, 액세스 토큰 원문을 로그에 남기지 않는다. |
