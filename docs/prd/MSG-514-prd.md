...
| FR-4 | 같은 화면에서 관심사가 다른 두 문장을 보내면, 각 관심사가 서로 다른 후보 집합에 걸리는 한 두 결과의 지점 구성이나 순서가 달라진다. 표기만 다르고 뜻이 같은 두 관심사가 같은 후보 집합에 걸리면 같은 동선이 옳고, 차이는 지점별 이유의 관심사 표기(FR-7)로 나타난다 (2026-08-31 정정, 성민 승인. 스펙 리뷰가 원 문면이 정당한 동치 케이스까지 위반으로 만드는 것을 적발) | Must |
