...
- [ ] **1. 버튼 순서.** 시안(피그마 `16015:540`)은 카카오 위, 애플 아래다. 애플 규정은 크기만
  요구하고 순서는 강제하지 않는 것으로 알고 있으나 원문 확인이 필요하다. 국내 앱은 대개
  카카오를 먼저 둔다.
- [ ] **2. 기본 닉네임 형식.** 이름이 안 올 때 붙일 닉네임. 후보는 "필맵러" 뒤에 짧은 무작위
  숫자. 닉네임은 중복 허용(FR-USER-03)이라 충돌 걱정은 없고, 2~20자 제약만 지키면 된다.
- [ ] **3. 애플 리프레시 토큰 보관.** `users`에 암호화 컬럼을 두는 안과 제공자 자격을 담는
