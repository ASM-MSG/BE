...
| ID | 요구사항 | 우선순위 |
|----|----------|----------|
| FR-1 | 위치 동의(FR-USER-14)와 OS 백그라운드 위치 권한을 모두 허용한 앱 사용자는, 활성 축제나 팝업의 근접 반경 안에 들어오면 촬영을 유도하는 알림을 받는다. 앱이 실행 중이 아니어도 받는다 | Must |
...
| FR-4 | 같은 미션의 근접 알림은 사용자당 1회다. 반경을 나갔다 다시 들어와도 반복 발화하지 않는다 | Must |
| FR-5 | 알림을 탭하면 해당 미션의 촬영 흐름으로 진입한다 | Must |
| FR-6 | 사용자는 이 알림을 설정에서 끌 수 있고, 위치 동의를 철회하거나 OS 권한을 회수하면 알림도 멈춘다. 권한이 없을 때는 실패가 아니라 조용한 미동작이다 | Must |
| FR-7 | 기기는 지오펜스 등록에 필요한 주변 활성 미션(축제, 팝업)의 위치와 식별 정보를 서버에서 받을 수 있다. 기존 뷰포트 미션 조회(FR-MISSION-01)를 재사용할지 전용 조회를 둘지는 스펙에서 정한다 | Must |
...
서버 신규 타입 없음이 기본안이다. 전용 조회를 두게 되면 그때 스펙에서 그린다.

## 7. 변경 파일 목록

| 파일 | 변경 | Owner |
|------|------|-------|
| (기본안) 없음 | 기존 뷰포트 미션 조회와 진행도 조회를 재사용하면 서버 변경 없음 | - |
| (대안) `src/main/java/com/msg/fillmap/mission/...` | 지오펜스 등록용 경량 조회 신설 시. 좌표 중심점과 근접 반경을 내려주는 형태 | B |
...
- [ ] 근접 반경 값. 판정 반경(팝업 40m)과는 다른, 유도용 반경이다. 축제와 팝업을 다르게 잡을지
  포함해서 값이 필요하다. 너무 크면 스팸이고 너무 작으면 발화가 안 된다.
- [ ] 하루 전체 상한. 미션당 생애 1회(FR-4)에 더해 하루 N건 상한을 둘지. 홍대처럼 팝업이 밀집한
  곳을 지나면 하루에 여러 건이 터질 수 있다(뷰포트 하나에 팝업 38개 실측, FR-MISSION-15).
...
- [ ] 디자인 부재. 앱 정본(ver 5)에 알림 관련 화면이 없다. 설정 토글, 권한 요청 온보딩, 알림
  ...
  설득 문구가 성패를 가른다.
- [ ] 로컬 알림이면 서버의 카테고리와 옵트아웃 테이블에 이 알림이 아예 안 잡힌다. 알림 설정을
  서버에 동기화할지 앱 로컬 설정으로 둘지.
