...
> **개정 (2026-08-19, MSG-433)**: 위치기반서비스 동의는 철회 불가로 전환됐다. 이 문서의 "동의 회수로 알림이 멈춘다" 경로 중 서버 동의 철회는 더는 성립하지 않고, OS 위치 권한 회수와 알림 설정 끄기 경로만 유효하다.
...
| FR-2 | 서버는 사용자 위치를 받아 판정하거나 수집·저장하지 않는다. 근접 판정은 기기의 지오펜싱[^1]이 하고, 서버 역할은 미션 위치 데이터 제공까지다. 미션 조회에 실리는 뷰포트 bbox는 위치에서 파생된 값이지만 지도 열람과 동일한 기존 조회 경로라 이 원칙의 위반으로 보지 않는다 (2026-08-18 기기 판정형 확정 · 같은 날 문언 정정) | Must |
...
- [x] FR-2 문언과 뷰포트 bbox의 간극: 문언 정정으로 확정 (2026-08-18 성민, 스펙 리뷰에서 발견).
  백그라운드 갱신의 bbox 4좌표는 기기 위치에서 파생되므로 원래 문언 "사용자 위치는 기기 밖으로
  나가지 않는다"와 문자 그대로 어긋났고, 그 문언대로면 기존 지도 화면의 조회부터 위반이었다.
  약속의 실질은 비목표 절 표현대로 "서버가 위치를 받아 판정·수집·저장하는 방식 배제"라 FR-2를
  그렇게 다듬었다. 기기가 bbox를 격자 단위로 스냅해 정밀도를 낮추는 완화는 필수로 걸지 않고
  FE 계약 논의 항목으로 남긴다.
