...
| 분류 | 요구사항 |
|------|----------|
| 성능 | 집계 쿼리의 최종 결과를 21행으로 제한해 20행과 다음 묶음 존재 여부를 결정한다. 전체 건수 조회를 추가하지 않고 응답 본문은 최대 20개 항목으로 제한한다. 전역 집계 자체의 비용은 배포 전 실행 계획으로 확인한다. **확인 결과 이 조건은 충족하지 못했다** (2026-08-22 dev 실측: 첫 페이지 3,833 ms, 깊은 페이지 3,675 ms로 페이지 깊이와 무관하게 전역 집계가 매번 반복된다). 응답 본문 크기 제한은 달성했으나 집계 비용 제한은 달성하지 못했고, 그 부분은 [MSG-461](https://soma17-msg.atlassian.net/browse/MSG-461)로 분리했다. **이 미충족을 안고 배포하는 것은 2026-08-22 정민 판단이다.** 수용하는 영향은 검색창을 눌렀을 때 뜨는 기본 목록이 약 3.7초 걸린다는 것이고, MSG-461을 다음 스프린트에 넣어 해소하는 것을 조건으로 한다.<br><br>**정정 (2026-08-25, MSG-461 분해 실측, 정민 승인)** — 위에 적은 "전역 집계가 매번 반복되는 것이 원인"은 잘못된 진단이었다. 쿼리를 조각내 다시 재 보니 집계 자체는 257 ms이고, 6.8초 중 나머지는 `regions` 조인을 집계보다 앞에 둬서 행정동 조회가 144,536번 돈 것(`Memoize loops=144536`)과, 그룹 키가 `(region_code, region_name)`으로 넓어 정렬이 디스크로 샌 것(`external merge Disk: 2312kB`)이었다. 따라서 처방도 "집계를 미리 계산해 두기"가 아니라 **"조인을 집계 뒤로 옮기기"**로 바뀐다. 집계를 요청마다 도는 것 자체는 허용한다.<br><br>**승인하는 성능 목표** — 이 목록의 첫 페이지와 깊은 페이지 응답이 **500 ms 미만**이고, 비용이 페이지 깊이에 따라 늘지 않는다(MSG-461 실측 272~345 ms). 집계가 격자 수에 비례하는 것은 남는 한계이므로, **격자 50만 초과 또는 이 엔드포인트 p95 1초 초과 시** 미리 계산(머티리얼라이즈드 뷰)으로 승격한다 |
