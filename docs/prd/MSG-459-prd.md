...
| FR-12 | 미션 데이터를 다시 적재해도 이미 저장된 대표 격자와 이미 올라온 영상은 영향을 받지 않는다. 다만 재적재로 판정 격자 집합이 바뀌어 기존 대표 격자가 그 집합 밖이 된 경우는 예외로, 새 집합에서 다시 산출한다[^3] | Must |
...
| FR-18 | 미션 경유 업로드는 요청에 담긴 촬영 시각이 미션 기간 안일 때만 받는다. 기간 밖이면 거절한다[^4] | Must |
...
| 성능 | 업로드 확정 경로에 미션 조회 한 번과 멱등 재시도 판정(잠금 한 문장과 확정 영상 조회 한 건 — 행사 업로드가 이미 같은 모양으로 지불하는 비용)이 늘어난다. 판정 쿼리 수는 기존과 같아야 한다 |
...
sequenceDiagram
    ...
    participant API as 미션 업로드 컨트롤러
...
| 미션 경유 업로드 엔드포인트를 받을 컨트롤러 | 신규 또는 기존 확장. **`MissionVideoController`라는 이름을 새로 만들 수 없다** - `com.msg.fillmap.video.controller.MissionVideoController`가 이미 있어(MSG-390 미션 영상 목록) 같은 이름을 다시 쓰면 빈 이름이 충돌해 기동이 실패한다. 배치와 이름은 스펙이 정한다 | B |
...
- **종료된 축제·팝업을 대표 격자에서 되짚어 볼 수 있게 한다** (FR-15~17, 2026-08-22 정민). 지금은 미션 조회가 활성 기간으로 걸러 끝난 축제가 화면에서 사라진다. 영상은 대표 격자에 남아 있는데 그것이 무슨 축제였는지 알 방법이 없다. 행사방이 격자 역조회로 지난 회차까지 돌려주는 것과 같은 모양으로 맞춘다.
  ...
  - 보존은 성립하지만 근거가 처음 쓴 것과 다르다 (2026-08-22 정정). 처음에는 "FR-8이 기간 지난 업로드를 막으므로 미션 경유로 영상이 올라왔다는 것은 곧 스탬프가 찍혔다는 뜻"이라고 적었는데, FR-8이 보는 것은 업로드 시각이고 스탬프 판정이 보는 것은 촬영 시각이라 고리가 끊긴다. 축제 기간 중에 지난달 영상을 골라 올리면 업로드는 통과하고 스탬프는 안 찍혀, 영상이 있는 미션이 종료 정리에 지워질 수 있었다. 스펙이 촬영 시각 가드로 이 고리를 다시 이었다(`docs/spec/MSG-459.md` D-9). 결론은 그대로다 - 정리 규칙을 손대지 않아도 영상이 있는 미션은 남는다.
...
[^4]: 요청에 담긴 촬영 시각. 서버는 영상 파일을 열어 촬영 시각을 확인하지 않는다. 클라이언트가 채워 보내는 값을 그대로 쓴다. 서버가 이 값에 하는 일은 두 가지인데 둘 다 값 자체를 대조하지는 못한다. 영상 도메인이 공통으로 거는 "업로드 시점보다 미래인가" 검사(MSG-278)와, 이 요구가 새로 거는 "미션 기간 안인가" 검사다. 앱이 갤러리 항목의 메타데이터에서 이 값을 채우는 것이 의도된 동작이지만 그것은 클라이언트 관례이지 서버가 보증하는 사실이 아니다. 그래서 이 요구는 위조를 막는 장치가 아니다. 요청을 직접 만들어 기간 안 시각을 적으면 통과한다. 이 요구가 실제로 하는 일은 업로드를 받는 판정과 스탬프를 주는 판정이 같은 값을 보게 만드는 것이다. 두 판정이 어긋나면 영상은 올라왔는데 스탬프가 없는 미션이 생기고, 그 미션은 기간이 끝날 때 지워져 FR-17이 깨진다. 사용자에게 보이는 효과는 하나다. 지난달에 찍은 갤러리 영상을 이번 축제의 영상으로 올리려 하면 거절된다.

[^1]: 판정 범위. 그 미션을 완료한 것으로 인정하는 격자들. 축제는 중심 좌표 기준 9×9이고 팝업은 좌표 사방 40m가 걸치는 칸이다. 표시할 크기가 아니라 인정할 범위다.
[^3]: FR-12 예외 (2026-08-22 정정). 처음 이 요구를 쓸 때 막으려던 것은 "같은 데이터를 다시 넣었는데 결과가 달라지는 일"이었다. 그런데 팝업이 실제로 자리를 옮기면 판정 격자 집합 자체가 바뀌고, 그때 옛 대표 격자를 고수하면 두 가지가 어긋난다. 저장 계층에서는 대표 격자가 판정 격자 집합에 속해야 한다는 제약이 깨져 재적재 트랜잭션 전체가 되돌아가고(그 주의 팝업이 통째로 노출되지 않는다), 사용자에게는 팝업이 없는 자리가 그 팝업의 영상이 모이는 자리로 남는다. 그래서 집합이 바뀐 경우에 한해 다시 산출한다. 집합이 그대로면 값도 그대로이므로 멱등이라는 원래 의도는 유지된다. 이 예외가 남기는 부작용(이미 올라온 영상은 옛 격자에 남고, 그 사이 재시도가 거절될 수 있다)은 스펙 D-3과 D-6에 한계로 기록돼 있다.
