...
클라이언트가 합성한다. 재료는 내 수집 격자(`GET /api/collections/grids`)와 내 스탬프이고,
미션 격자와의 교집합이 진행도다. 스탬프는 완료 여부다. 이렇게 두는 이유는 목록에 전역 캐시가
...
합계라 같은 계산에서 나온다. 축제와 팝업의 "0/1칸"은 미션 격자와 내 수집 격자의 교집합 개수다.
