...
| FR-9 | 근접 미션(MISSION_NEARBY) 알림은 목록에 나타나지 않는다. 기기가 로컬로 만드는 알림이라 서버 이력에 행 자체가 없고, V36 마이그레이션이 CHECK 제약으로 이를 강제하고 있다. 별도 구현 없이 성립하는 사실이지만, FE와 디자인이 전제로 삼도록 명시한다 | Must |
