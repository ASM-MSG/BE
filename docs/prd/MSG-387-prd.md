...
| FR-5 | 헤더 카운트(gridCount, videoCount)는 limit과 무관한 전체 기준이므로, 카드가 20장으로 잘려도 "격자 N개 · 영상 M개"는 전체 수를 보여준다 (기존 계약 유지) | Must |
...
| 분류 | 요구사항 |
|------|----------|
| 데이터 정합 | 카드 집합과 헤더 카운트는 같은 조회의 한 스냅샷에서 나와 서로 어긋나지 않는다 (기존 동작 유지) |
...
sequenceDiagram
    ...
    API-->>FE: 헤더 카운트(전체 기준) + 카드 최대 20장(최신순)
    FE-->>U: 사이드 패널 카드 렌더
    U->>FE: "전체 보기" 클릭
