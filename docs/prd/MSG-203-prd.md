...
카카오 로그인 시 닉네임은 id_token 클레임에서 자동 저장되고 있는데, 정작 사용자가
...
- **목표**:
  - 로그인 사용자가 자기 이메일·닉네임을 조회할 수 있다 — 단, **카카오에서 이메일은 받지 않는다**
    (2026-08-03 후속 결정, MSG-310). 카카오 유저의 email 은 null 이고 이메일 가입(로컬 테스트용)
    유저만 값이 있다 — 조회 응답의 email 은 null 가능 필드
