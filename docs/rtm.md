...
AC 요약: 스펙 성공 기준 32건 중 테스트 연결 30건, 미커버 2건 — 미커버는 보고만 한다 (소급 없이 신규 스펙부터 쌓인다, MSG-526)
...
| FR-VIDEO-03 | 구현됨 | VideoPresignTest, VideoSignatureIntegrationTest, VideoSignatureTest |
...
| FR-MISSION-19 | 진행 중 | CourseMissionSeederIntegrationTest, CourseSeedContractTest, CourseSeedReaderTest, CourseSpotNameResolverTest |
...
| FR-ROUTE-13 | 구현됨 | RouteCandidateCollectorTest, RouteOrderPlannerTest, RouteRecommendServiceTest |
...
| FR-FRIEND-13 | 계획 | (없음) |
| FR-FRIEND-14 | 구현됨 | FriendControllerTest, FriendIntegrationTest |
...
스펙 「성공 기준」의 번호 항목(`- AC-{티켓}-{순번}: 판정문`)과 테스트 마커(`// 검증: ..., AC-...`)를 잇는다.
FR 축과 다른 축이라 표를 섞지 않는다. 미커버는 조치 유도용 보고이고 CI 를 깨뜨리지 않는다.
...
## 형식이 어긋난 AC 마커 토큰 (매핑 집계에서 무시됨)

(없음)

## 중복 정의된 AC ID (같은 번호가 두 번 이상 정의됨 — 파일 내 복붙·파일 간 충돌 모두)
